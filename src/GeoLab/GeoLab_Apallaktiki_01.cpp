#include "GeoLab_Apallaktiki_01.h"
#include "utils.h"
#include "canvas.h"
#include <iostream>
#include <fstream>
    ...
    for (int ci=0; ci<m_pts.size(); ci++)
    {
        for (int pi=0; pi<m_pts[ci].size(); pi++)
        {
            ...
            else
            {
                ...
                Colour col = m_active_contour==ci ? Colour::red:Colour::yellow;
                Triangle3D(p1.x,p1.y,p1.z, p2.x,p2.y,p2.z, p2z.x,p2z.y,p2z.z, col).draw();
                Triangle3D(p1.x,p1.y,p1.z, p2z.x,p2z.y,p2z.z, p1z.x,p1z.y,p1z.z, col).draw();
